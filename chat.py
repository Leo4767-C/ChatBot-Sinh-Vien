...
async def _stream_text_answer(
    *,
    ...
    # --- Đã ẩn việc tự động show ảnh lúc đầu đi ---
    ...
    # --- PHẦN TỐI ƯU HÓA HIỂN THỊ ẢNH THEO TỪ KHÓA ---
    # Lấy sẵn danh sách ảnh trong các đoạn tài liệu tìm được
    available_images_in_context = _collect_data_images(chunks, limit=5)
    final_selected_images = []

    if available_images_in_context:
        # Tập hợp các từ khóa kích hoạt hiển thị ảnh (Thêm từ khóa liên quan đến data của bạn)
        keyword_map = ["bảng", "biểu đồ", "hình", "sơ đồ", "cơ cấu", "kiến trúc", "kết quả", "phân tích", "ielts", "điểm"]
        text_lower = full.lower()
        
        # Chỉ hiển thị ảnh nếu nội dung câu trả lời chứa ít nhất 1 từ khóa
        if any(kw in text_lower for kw in keyword_map):
            final_selected_images = available_images_in_context[:1] # Lấy 1 bức liên quan nhất
            yield f"data: [IMAGES]{json.dumps(final_selected_images, ensure_ascii=False)}\n\n"

    # Lưu db bằng danh sách ảnh đã qua chọn lọc
    db.add(ChatMessage(
        session_id=session_id,
        role="model",
        content=full,
        source_docs=_pack_meta(sources=source_names, images=final_selected_images, kind="rag"),
    ))
    await db.commit()
